####################################################
## Prepare the vgg model for the image level classifier and the part level classifier.
###################################################
...
def make_layers(cfg, batch_norm=False):
    layers = []
    in_channels = 3
    print('the vgg network structure has been changed')
    for v in cfg:
        if v == 'M':
...
def vgg16_bn(args, **kwargs):
    ...
    if args.pretrained_model != '':
        print('load the pretrained model from:', args.pretrained_model)
        pretrained_dict = torch.load(args.pretrained_model)['state_dict']
        pretrained_dict_temp = copy.deepcopy(pretrained_dict)
        model_state_dict = model.state_dict()

        for k_tmp in pretrained_dict_temp.keys():
            if k_tmp.find('module.') != -1:
                k = k_tmp.replace('module.', '')
    ...
    source_model = Share_convs(model, 512, args.num_classes_t)
    return source_model
...
class Share_convs(nn.Module):
    def __init__(self, base_conv, convout_dimension, num_class):
...
def vgg19_bn(args, **kwargs):
    ...
    source_model = Share_convs(model, 512, args.num_classes)
    return source_model
